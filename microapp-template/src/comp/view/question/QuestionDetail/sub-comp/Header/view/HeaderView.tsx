      ...
      <Card.Header
        ...
        subheader={(
          <Box pt={2} display="flex" alignItems="center">
            <Box mr={3} display="flex" color="text.secondary">
              <Box pr={1}>
                <PersonIcon fontSize="small" />
              </Box>
              <Typography variant="body2">
                {this.writerName(question.secret, question.writerName)} <Typography variant="caption" bold color="textSecondary">(나)</Typography>
              </Typography>
            </Box>
            {!question.anonymous && (
        ...
        action={renderAction && renderAction(question)}
      />
    );
  }
}
